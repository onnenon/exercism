defmodule TakeANumberDeluxe do
  ...
  def init(init_arg) do
    with timeout <- Keyword.get(init_arg, :auto_shutdown_timeout, :infinity),
         min_number <- Keyword.get(init_arg, :min_number),
         max_number <- Keyword.get(init_arg, :max_number),
         {:ok, state} <- TakeANumberDeluxe.State.new(min_number, max_number, timeout) do
      {:ok, state, timeout}
    else
      e -> e
    end
  ...
  def handle_call(:queue_new_number, _from, state) do
    with {:ok, number, new_state} <- TakeANumberDeluxe.State.queue_new_number(state) do
      {:reply, {:ok, number}, new_state, state.auto_shutdown_timeout}
    else
