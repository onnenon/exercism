defmodule Poker do
  ...
  defp compare_by_high_cards(hand1, hand2) do
    ranks1 = hand1.cards |> Enum.map(& &1.rank) |> Enum.sort(&>=/2)
    ranks2 = hand2.cards |> Enum.map(& &1.rank) |> Enum.sort(&>=/2)
  ...
  defp four_of_a_kind?(ranks) do
    rank_counts = Enum.frequencies(ranks)
    Enum.any?(rank_counts, &(elem(&1, 1) == 4))
  end
  ...
  defp three_of_a_kind?(ranks) do
    ...
    Enum.any?(rank_counts, &(elem(&1, 1) == 3)) &&
      Enum.count(rank_counts) == 3
  end

  defp two_pair?(ranks) do
    rank_counts = Enum.frequencies(ranks)
    Enum.count(rank_counts, &(elem(&1, 1) == 2)) == 2
  end

  defp pair?(ranks) do
    rank_counts = Enum.frequencies(ranks)
    Enum.count(rank_counts, &(elem(&1, 1) == 2)) == 1
  end

  defp hand_category_to_rank(category) do
    case category do
      :straight_flush -> 9
      :four_of_a_kind -> 8
      :full_house -> 7
