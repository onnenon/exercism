defmodule Poker do
  ...
  defp get_category(cards) do
    ...
    cond do
      is_straight_flush?(ranks, suits) -> :straight_flush
      is_four_of_a_kind?(ranks) -> :four_of_a_kind
      is_full_house?(ranks) -> :full_house
      is_flush?(suits) -> :flush
      is_straight?(ranks) -> :straight
      is_three_of_a_kind?(ranks) -> :three_of_a_kind
      is_two_pair?(ranks) -> :two_pair
      is_pair?(ranks) -> :pair
      true -> :high_card
    end
  end

  defp is_straight_flush?(ranks, suits) do
    is_straight?(ranks) && is_flush?(suits)
  end

  defp is_four_of_a_kind?(ranks) do
    rank_counts = Enum.frequencies(ranks)
    Enum.any?(rank_counts, fn {_rank, count} -> count == 4 end)
  end

  defp is_full_house?(ranks) do
    rank_counts = Enum.frequencies(ranks)
  ...
  defp is_three_of_a_kind?(ranks) do
    rank_counts = Enum.frequencies(ranks)
