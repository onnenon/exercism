...
pub fn root_for_team(team: Team) -> Bool {
  case team {
    ...
    Team(
      coach: Coach(name: "Gregg Popovich", former_player: _),
      name: _,
      stats: _,
    ) -> True
    Team(coach: Coach(name: _, former_player: True), name: _, stats: _) -> True
    Team(stats: Stats(wins: w, losses: _), coach: _, name: _) if w >= 60 -> True
    Team(stats: Stats(wins: w, losses: l), coach: _, name: _) if l > w -> True
    _ -> False
  }
}
